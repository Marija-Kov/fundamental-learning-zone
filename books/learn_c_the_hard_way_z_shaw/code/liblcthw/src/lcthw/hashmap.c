...
static inline DArray *Hashmap_find_bucket(Hashmap *map, void *key, int create, uint32_t *hash_out)
{
    ...
    // store it for the caller
    *hash_out = hash;
    // we expect to find a bucket in the input map
...
// can't we get the bucket from map->buckets?
static inline int Hashmap_get_node(Hashmap *map, uint32_t hash, DArray *bucket, void *key)
{
    ...
    for (i = 0; i < DArray_end(bucket); i++) {
        ...
        if (node->hash == hash && map->hash && map->compare(node->key, key) == 0) {
            return i;
        }
    }
    return -1;
...
void *Hashmap_delete(Hashmap *map, void *key)
{
    ...
    if (ending != node) {
        // If the last node isn't the deleted node itself, 
        // move it in the deleted node's place in the bucket:
