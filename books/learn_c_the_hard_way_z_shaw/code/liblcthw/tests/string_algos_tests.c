...
char *test_find_and_scan()
{
    ...
    mu_assert(scan_i > 0, "Failed to find pattern with scan.");

    mu_assert(scan_i == find_i, "Find and scan results don't match.");

    scan_i = StringScanner_scan(scan, &ALPHA);
    mu_assert(scan_i > find_i, "Should have found the second occurence of the pattern.");

    scan_i = StringScanner_scan(scan, &ALPHA);
    mu_assert(scan_i > find_i, "Should have found the third occurence of the pattern.");

    mu_assert(StringScanner_scan(scan, &ALPHA) == -1, "Should not have found another occurence of the pattern.");
